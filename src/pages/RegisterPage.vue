...
<template>
  <Card class="w-full max-w-sm mx-auto border-border/60 shadow-xl">
    <CardHeader class="space-y-1 pb-4 text-center">
      ...
      <CardDescription>Start managing your finances with SpendWise</CardDescription>
    </CardHeader>

    <CardContent class="space-y-4">
      <ScrollArea class="max-h-[55vh] pr-1">
        <form @submit.prevent="onSubmit" class="space-y-4 pb-1">
          <!-- Full Name -->
          <div class="space-y-1.5">
            <Label for="name">Full Name</Label>
            <div class="relative">
              <User class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
              <Input id="name" v-model="formData.name" placeholder="Your name" class="pl-9" required />
            </div>
          </div>
          ...
          <div class="space-y-1.5">
            ...
            <div class="relative">
              ...
              <Input id="reg-email" v-model="formData.email" type="email" placeholder="you@example.com"
                class="pl-9" required />
            </div>
          </div>
          ...
          <div class="space-y-1.5">
            ...
            <div class="relative">
              ...
              <Input id="reg-password" v-model="formData.password" :type="isPwd ? 'password' : 'text'"
                placeholder="Min 8 characters" class="pl-9 pr-10" required />
              <button type="button"
          ...
          <div class="space-y-1.5">
            ...
            <div class="relative">
              ...
              <Input id="confirm-password" v-model="formData.password_confirmation"
                :type="isPwdConfirm ? 'password' : 'text'" placeholder="Repeat password"
                class="pl-9 pr-10" required />
              <button type="button"
                class="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground transition-colors"
          ...
          <div class="flex items-start gap-2.5">
            <Checkbox id="terms" :checked="formData.terms" class="mt-0.5"
            ...
            <Label for="terms" class="text-sm font-normal leading-snug cursor-pointer text-muted-foreground">
              I agree to the
              <button type="button" class="text-indigo-500 hover:underline font-medium"
                @click="showTermsDialog = true">Terms of Service</button>
              and
              <button type="button" class="text-indigo-500 hover:underline font-medium"
                @click="showPrivacyDialog = true">Privacy Policy</button>
            </Label>
          </div>

          <!-- Submit -->
          <Button type="submit" class="w-full bg-indigo-600 hover:bg-indigo-500 text-white" size="lg">
      ...
      <!-- Google -->
      <Button type="button" variant="outline" class="w-full gap-2" @click="socialLogin('Google')">
  ...
  <Dialog v-model:open="showTermsDialog">
    <DialogContent class="sm:max-w-md">
      ...
      <DialogFooter>
        <Button @click="showTermsDialog = false">Close</Button>
      </DialogFooter>
    </DialogContent>
  </Dialog>
  ...
  <Dialog v-model:open="showPrivacyDialog">
    <DialogContent class="sm:max-w-md">
      ...
      <DialogFooter>
        <Button @click="showPrivacyDialog = false">Close</Button>
      </DialogFooter>
    </DialogContent>
  </Dialog>
</template>
